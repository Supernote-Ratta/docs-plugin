...
For details, see [`Element`](/en/api-reference/supernote-plugin/types/trail). The created `Element` is cached on the native side — remember to release it with [`recycleElement`](/en/api-reference/supernote-plugin/plugin-comm-api/recycle-trail) or `element.recycle()` once you're done; see [Element Caching and Release](/en/plugin-base/file-op/element-op#4-element-caching-and-release) for the full workflow.
