...
| `numInPage` | `number` | Index within the page (starts from `1`) |
...
| `picture` | [`Picture \| null`](/en/api-reference/supernote-plugin/types/picture) | Picture data (only when `type === 200`; `picturePath` only supports `png`, `jpg`, and `jpeg` formats) |
...
<Tip>
`element.recycle()` is equivalent to `PluginCommAPI.recycleElement(element.uuid)` — it's just called directly on the `Element` instance instead. Either form works; see [`recycleElement`](/en/api-reference/supernote-plugin/plugin-comm-api/recycle-trail) and [Element Caching and Release](/en/plugin-base/file-op/element-op#4-element-caching-and-release) for details.
</Tip>
