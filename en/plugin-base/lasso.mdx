...
| Flip selected elements | [`flipLassoElements(flipType)`](/en/api-reference/supernote-plugin/plugin-comm-api/flip-lasso-elements) | Flips supported elements in the current lasso selection. `1` is horizontal and `2` is vertical |
...
- `flipLassoElements` only flips strokes and geometries, and currently supports horizontal and vertical flipping only
