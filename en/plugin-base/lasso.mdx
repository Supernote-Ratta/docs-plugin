...
| Resize lasso rectangle | [`resizeLassoRect(rect, rotationDegree?)`](/en/api-reference/supernote-plugin/plugin-comm-api/resize-lasso-rect) | Resizes the lasso rectangle and can optionally rotate supported elements |
| Duplicate and flip selected elements | [`duplicateFlipLassoElements(flipType)`](/en/api-reference/supernote-plugin/plugin-comm-api/duplicate-flip-lasso-elements) | Duplicates supported elements in the current lasso selection, then flips the new elements. `1` is horizontal and `2` is vertical |
...
| Control lasso box | [`setLassoBoxState(state)`](/en/api-reference/supernote-plugin/plugin-comm-api/set-lasso-box-state) | `0` show, `1` hide, `2` remove completely, `3` fully hide (including lasso elements) |
...
- `resizeLassoRect` optionally accepts `rotationDegree`: omit it to keep the current rotation; when provided, only strokes and geometries rotate
- During `resizeLassoRect`, only titles, strokes, and geometries support stretch resizing; all other element types support proportional scaling only
- `duplicateFlipLassoElements` first duplicates supported elements, then flips the newly created elements; the originals remain unchanged
- `duplicateFlipLassoElements` only supports strokes and geometries, and currently supports horizontal and vertical flipping only
