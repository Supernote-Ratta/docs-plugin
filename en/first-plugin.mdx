...
| `uses-permissions` | Optional declaration of the permissions the plugin needs, as an array of permission name strings, e.g. `["plugin.permission.FILE:WRITE", "plugin.permission.INTERNET"]`. A permission must be declared here before calling [`hasPermission`](/en/api-reference/supernote-plugin/plugin-manager/has-permission)/[`requestPermission`](/en/api-reference/supernote-plugin/plugin-manager/request-permission) for it; otherwise the call fails. See [Plugin Permissions](/en/plugin-base/permission). |

Like `author`, `uses-permissions` is not generated by the packaging script and must be added manually. Example `PluginConfig.json` after manually adding `uses-permissions`:
...
```json wrap
{
  ...
  "versionName": "0.0.1",
  "jsMainPath": "index",
  "uses-permissions": ["plugin.permission.FILE:WRITE", "plugin.permission.INTERNET"]
}
```
