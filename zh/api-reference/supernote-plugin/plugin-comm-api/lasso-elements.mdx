...
传入的 `rect` 必须位于页面尺寸范围内：`left` 和 `top` 可以等于 `0`，`right` 和 `bottom` 可以等于页面最大尺寸，但不能超出页面边界。页面尺寸请通过 [`PluginFileAPI.getPageSize`](/zh/api-reference/supernote-plugin/plugin-file-api/get-page-size) 获取。
</Note>
...
| 参数 | 类型 | 说明 |
| --- | --- | --- |
| `rect` | [`Rect`](/zh/api-reference/supernote-plugin/types/rect) | 套索矩形（像素坐标）。矩形区域必须在页面尺寸范围内，`left/top` 可等于 `0`，`right/bottom` 可等于页面最大尺寸 |
