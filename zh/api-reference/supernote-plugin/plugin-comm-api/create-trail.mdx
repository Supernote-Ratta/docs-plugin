...
详细原因见[`Element`](/zh/api-reference/supernote-plugin/types/trail)。创建出的 `Element` 会被缓存在原生侧，用完记得调用 [`recycleElement`](/zh/api-reference/supernote-plugin/plugin-comm-api/recycle-trail) 或 `element.recycle()` 释放，完整流程见 [Element 缓存与释放](/zh/plugin-base/file-op/element-op#4-element-缓存与释放)。
