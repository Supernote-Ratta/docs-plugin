...
| `status` | `number` | 元素状态值 |
| `numInPage` | `number` | 元素在页内的序号（从 `1` 开始） |
...
| `picture` | [`Picture \| null`](/zh/api-reference/supernote-plugin/types/picture) | 图片数据（仅 `type === 200` 时存在；其中 `picturePath` 仅支持 `png`、`jpg`、`jpeg` 格式） |
...
<Tip>
`element.recycle()` 等价于 `PluginCommAPI.recycleElement(element.uuid)`，只是换成了在 `Element` 实例上直接调用，二者选其一即可，详见 [`recycleElement`](/zh/api-reference/supernote-plugin/plugin-comm-api/recycle-trail) 与 [Element 缓存与释放](/zh/plugin-base/file-op/element-op#4-element-缓存与释放)。
</Tip>
